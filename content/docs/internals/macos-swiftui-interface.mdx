---
title: macOS SwiftUI interface
description: KumoApp's scene structure, navigation model, toolbar behavior, Settings, status item, App Intents, and the UI constraints that keep daily workflows first.
---
...
Preferences persist to `~/Library/Application Support/Kumo/preferences.json` via `UserPreferencesStore`. See [Persistence and Logging](/docs/internals/persistence-logging) for fields.
