...
macOS asks for **administrator authorization** in a few narrowly
...
- **App Sandbox** — **disabled** in v1. Spawning Mihomo and Sub-Store
  ...
  plus helper-bundle separation is on the roadmap.
...
directly, so it does not trigger the prompt.
...
<Callout type="info">
If something else asks for root, verify what is actually triggering it.
Kumo's prompts always come from the **Kumo** application (visible in
the auth dialog).
</Callout>
