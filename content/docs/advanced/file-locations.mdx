...
Decoding tolerates missing fields, so a Kumo update can add new keys
...
<Callout type="info">
Do not run `make reset-local-state` against a real install unless the
intention is to erase everything. Re-running first-launch onboarding is
a side effect.
</Callout>
